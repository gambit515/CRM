...
class FilialDetailAPIView(APIView):
    def get(self, request, id):
        try:
            record = Filial.objects.get(id=id)
        except Filial.DoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)

        serializer = FilialSerializer(record)
    ...
    def put(self, request, id):
        ...
        serializer = FilialSerializer(record, data=request.data)
...
class ProcedureDetailAPIView(APIView):
    def get(self, request, id):
        try:
            record = Procedure.objects.get(id=id)
        except Procedure.DoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)

        serializer = ProcedureSerializer(record)
        return Response(serializer.data)
    ...
    def put(self, request, id):
        ...
        serializer = ProcedureSerializer(record, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ScheduleCreateAPIView(generics.ListCreateAPIView):
    serializer_class = RecordLiteSerializer
    permission_classes = (IsAuthenticated,)


class ScheduleDeleteAPIView(generics.RetrieveUpdateDestroyAPIView):
...
class ScheduleDetailAPIView(APIView):
    def get(self, request, id):
        ...
        serializer = RecordLiteSerializer(record)
        return Response(serializer.data)
    ...
    def put(self, request, id):
        ...
        serializer = RecordLiteSerializer(record, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
